...
function Project(){
    ...
    const cardData = [{
        ...
        description: "Blogging Website for topics concerning Agriculture",
    }, {
        title: "E-Learning Quiz Website",
        image: react,
        ...
        description: "Learning Online with Gamification Aspects"
    }];

    return (
        <React.Fragment>
            <Modal isOpen={isOpen} onClose={handleCloseModal}>
                <img src={modalChild} alt='project_image'/>
            </Modal>
            <div className='Project-container flex-center pad-side-2'>
                <h1 className={`title-text ${titleInView ? 'in-view' : ''}`} ref={titleRef}>Projects</h1>
                <div className={`options ${cardInView ? 'in-view' : ''}`} ref={cardRef}>
                    {cardData.map((card, index) => 
                        <div 
                            key={index} 
                            className={`option ${index === activeIndex ? 'active' : ''}`}  
                            style={{backgroundImage: `url(${card.image})`}}
                            onClick={() => handleClick(index, card.image)}
                        >
                            ...
                            <div className="label">
                                ...
                                <div className="info flex-center">
                                    ...
                                    <div className="sub">{card.description}</div>
                                </div>
                            </div>
                        </div>
                    )};
